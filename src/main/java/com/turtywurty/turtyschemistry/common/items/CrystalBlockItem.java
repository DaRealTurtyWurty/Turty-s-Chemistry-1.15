...
public class CrystalBlockItem extends BlockItem {
    ...
    public Entity createEntity(final World world, final Entity location, final ItemStack itemstack) {
        if (!world.isRemote) {
            final ItemEntity item = (ItemEntity) EntityTypeInit.FIRE_RES_ITEM.get().spawn((ServerWorld) world,
                    itemstack, null, location.getPosition(), SpawnReason.NATURAL, false, false);
            item.setItem(itemstack);
            item.setMotion(location.getMotion().getX(), location.getMotion().getX(),
                    location.getMotion().getX());
