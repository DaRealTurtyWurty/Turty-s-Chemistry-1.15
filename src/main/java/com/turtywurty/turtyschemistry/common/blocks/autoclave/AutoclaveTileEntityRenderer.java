...
public class AutoclaveTileEntityRenderer extends TileEntityRenderer<AutoclaveTileEntity> {
    ...
    public void render(final AutoclaveTileEntity tileEntityIn, final float partialTicks,
        ...
        final Direction direction = tileEntityIn.getBlockState().get(BaseHorizontalBlock.HORIZONTAL_FACING);
        for (int index = 0; index < tileEntityIn.size; ++index) {
            final ItemStack itemstack = tileEntityIn.getItemInSlot(index);
            if (itemstack != ItemStack.EMPTY) {
                ...
                matrixStackIn
                        .rotate(Vector3f.YP.rotationDegrees(direction.getOpposite().getHorizontalAngle()));
