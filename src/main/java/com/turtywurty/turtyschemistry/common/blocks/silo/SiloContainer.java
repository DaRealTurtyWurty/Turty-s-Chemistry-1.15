...
public class SiloContainer extends Container {
    ...
    // Server
    public SiloContainer(final int id, final PlayerInventory playerInventory, final BlockPos pos,
            final IItemHandler slots, final IIntArray data, final int page) {
        ...
        // Silo Inventory
        final int siloStartX = 13;
        final int siloStartY = 18;

        for (int row = 0; row < 8; row++) {
            for (int column = 0; column < 13; column++) {
                final SiloSlotItemHandler slot = new SiloSlotItemHandler(slots,
                        page * 104 + column + row * 13, siloStartX + column * slotSizePlus2,
                        siloStartY + row * slotSizePlus2);
                addSlot(slot);
            }
        }
    }

    public static SiloContainer getClientContainer(final int id, final PlayerInventory playerInventory) {
        return new SiloContainer(id, playerInventory, BlockPos.ZERO, new ItemStackHandler(624),
                new IntArray(1), 0);
    }

    public static IContainerProvider getServerContainerProvider(final SiloTileEntity te,
            final BlockPos activationPos, final int page) {
        tile = te;
