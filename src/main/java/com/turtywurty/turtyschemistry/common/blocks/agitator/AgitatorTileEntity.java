		...
		if (!this.world.isRemote) {
			for (int slot = 0; slot < getSize() - 2; slot++) {
				if (getItemInSlot(slot).getItem() instanceof BucketItem) {
					BucketItem bucket = (BucketItem) getItemInSlot(slot).getItem();
					if (!bucket.getFluid().equals(Fluids.EMPTY) && getFluidHandler().getFluidInTank(slot).isEmpty()) {
						getFluidHandler().fill(slot, new FluidStack(bucket.getFluid(), 1000), FluidAction.EXECUTE);
			...
			if (!getFluidHandler().isEmpty()) {
				for (AgitatorData recipe : getRecipes()) {
			...
			if (!getFluidHandler().getFluidInTank(5).isEmpty() && getItemInSlot(7).getItem() instanceof BucketItem) {
				ItemStack stack = FluidUtil.getFluidHandler(getItemInSlot(7)).map(handler -> {
