...
public class BalerTileEntityRenderer extends TileEntityRenderer<BalerTileEntity> {
    ...
    @Override
    public void render(final BalerTileEntity tileEntityIn, final float partialTicks,
    ...
    private void renderBale(final BalerTileEntity tile, final MatrixStack stackIn,
            final IRenderTypeBuffer bufferIn, final int combinedLightIn) {

        final Direction direction = tile.getBlockState().get(BaseHorizontalBlock.HORIZONTAL_FACING);
    ...
    private void renderPress(final BlockRendererDispatcher blockRenderer, final MatrixStack stackIn,
        ...
        if (tile.isCompressing()) {
            final float currentTime = tile.getWorld().getGameTime() + partialTicks;
