...
public class BriquettingPressTileEntity extends LockableLootTileEntity
    ...
    public void tick() {
        ...
        if (!isEmpty() && this.buttonPressed) {
            ...
            if (this.currentRunningTime % 60 == 0) {
                ...
                final Direction side = Direction.fromAngle(
                        getBlockState().get(BaseHorizontalBlock.HORIZONTAL_FACING).getHorizontalAngle() + 90);

                final IInventory inv = TileEntityUtils.getValidContainer(this.world, side, this.pos,
                        getBlockState(), new ItemStack(ItemInit.SAWDUST.get()));
                if (inv != null) {
                    TileEntityUtils.insertItem(inv, new ItemStack(ItemInit.BRIQUETTE.get()));
                } else {
                    final ItemEntity item = new ItemEntity(this.world, this.pos.offset(side).getX(),
    ...
    private boolean moveItems() {
        for (int i = 0; i < this.items.size(); i++) {
            if (!this.items.get(i).isEmpty() && i + 1 != this.items.size()
                ...
                if (this.items.get(i + 1).isItemEqual(this.items.get(i))
                    ...
                    final int availableSpace = getInventoryStackLimit() - this.items.get(i + 1).getCount();
                    final int neededSpace = this.items.get(i).getCount();
                    if (availableSpace >= neededSpace) {
                        this.items.set(i, ItemStack.EMPTY);
                        this.items.get(i + 1).grow(neededSpace);
                    } else {
                        final int newAmount = neededSpace - availableSpace;
