...
public class AgitatorContainer extends Container {
    ...
    public static AgitatorContainer getClientContainer(final int id, final PlayerInventory playerInventory) {
        return new AgitatorContainer(id, playerInventory, new ItemStackHandler(8), BlockPos.ZERO,
                new IntArray(1), AgitatorType.LIQUID_ONLY);
    }
    ...
    public static IContainerProvider getServerContainerProvider(final AgitatorTileEntity te,
        ...
        tile = te;
        return (id, playerInventory, serverPlayer) -> new AgitatorContainer(id, playerInventory,
                te.getInventory(), activationPos, new AgitatorSyncData(te), type);
    }
                ...
                for (final IContainerListener listener : this.listeners) {
                    if (listener instanceof ServerPlayerEntity) {
                        TurtyChemistry.PACKET_HANDLER.send(
                                ...
                                new AgitatorFluidPacket(tile.getFluidHandler().getFluidInTank(5),
                                        this.windowId));
                    }
                }
            }
