...
public class ElectrolyzerTileEntity extends InventoryTile {
    ...
    protected DumpMode dumpingInput = DumpMode.NONE, dumpingOutput1 = DumpMode.NONE,
            dumpingOutput2 = DumpMode.NONE;
            ...
            if (getItemInSlot(tank).getItem().equals(ItemInit.GAS_CANISTER_S.get())
                ...
                if (nbt.contains("GasName")) {
                    if (nbt.getInt("GasStored") < nbt.getInt("MaxAmount")
                        ...
                        this.fluidInventory.drain(tank,
                                new FluidStack(this.fluidInventory.getFluidInTank(tank), 1),
                                FluidAction.EXECUTE);
                    }
                } else {
    ...
    private boolean extractBucket() {
        if (getItemInSlot(0).getItem() instanceof BucketItem
            ...
            if (bucket.getFluid() != Fluids.EMPTY && bucket.getFluid() != null
                    ...
                    && this.fluidInventory.fill(0, new FluidStack(bucket.getFluid(), 1000),
    ...
    private ElectrolyzerRecipe getRecipe() {
        ...
        try {
            recipeOptional = getRecipes().stream()
                    .filter(recipe -> this.fluidInventory.isFluidEqual(0, recipe.input)).findFirst();
