...
public class ResearcherTileEntityRenderer extends TileEntityRenderer<ResearcherTileEntity> {
    ...
    public void render(final ResearcherTileEntity tileEntityIn, final float partialTicks,
        ...
        matrixStackIn.push();
        final Direction facing = tileEntityIn.getBlockState().get(ResearcherBlock.FACING);
        switch (facing) {
        case EAST:
        ...
        ClientUtils.MC.getBlockRendererDispatcher().getBlockModelRenderer().renderModel(
                tileEntityIn.getWorld(), model, tileEntityIn.getBlockState(), tileEntityIn.getPos(),
                matrixStackIn, bufferIn.getBuffer(RenderType.getSolid()), true,
                tileEntityIn.getWorld().getRandom(), ThreadLocalRandom.current().nextLong(),
        ...
        if (!tileEntityIn.getItemInSlot(1).isEmpty()) {
            renderItem(tileEntityIn.getItemInSlot(1), matrixStackIn, bufferIn, combinedLightIn,
                    combinedOverlayIn);
        } else if (!tileEntityIn.getItemInSlot(0).isEmpty()) {
            renderItem(tileEntityIn.getItemInSlot(0), matrixStackIn, bufferIn, combinedLightIn,
                    combinedOverlayIn);
        }
        matrixStackIn.pop();
        ...
        if (tileEntityIn.getProcessingTime() > 0) {
            ...
            BeaconTileEntityRenderer.renderBeamSegment(matrixStackIn, bufferIn,
                    ...
                    tileEntityIn.getWorld().getGameTime(), 0, 1, new float[] { 1.0f, 0.3f, 0.2f }, 0.025f,
                    0.0275f);
            matrixStackIn.pop();
        }
    }
