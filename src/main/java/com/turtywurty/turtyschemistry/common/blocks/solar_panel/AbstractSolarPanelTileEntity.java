...
public abstract class AbstractSolarPanelTileEntity extends TileEntity implements ITickableTileEntity {
	...
	public TurtEnergyStorage createEnergyStorage(final int capacity, final int maxReceive, final int maxExtract,
			final int defaultEnergy, final boolean canExtract, final boolean canRecieve) {
	...
	public void generatePower() {
		...
		if (getEnergyStorage().getEnergyStored() != getEnergyStorage().getEnergyStored()
			...
			getEnergyStorage()
					.setEnergy(getEnergyStorage().getEnergyStored() + i * getBlock().getPanelInfo().get().perTick());
			markDirty();
		}
	}
