...
public class GasExtractorScreen extends ContainerScreen<GasExtractorContainer> {
    ...
    public void render(final MatrixStack stack, final int mouseX, final int mouseY,
        ...
        renderGasTooltip(stack, "Helium Gas: " + this.container.tileEntity.getGasStored() + "mb", mouseX,
                mouseY);
    }
    ...
    protected void drawGuiContainerForegroundLayer(final MatrixStack stack, final int mouseX,
        ...
        final int storedAmount = this.container.tileEntity.getGasStored() / 1000;
        final boolean full = storedAmount == 3;

        getMinecraft().getTextureManager().bindTexture(BACKGROUND_TEXTURE);

        ClientUtils.blit(stack, this, 154, 66 - storedAmount * 20, 127, full ? 166 : 229 - storedAmount * 20,
                14, 17 + storedAmount * 20);

        ClientUtils.blit(stack, this, 7, 66 - storedAmount * 20, 127, full ? 166 : 229 - storedAmount * 20,
                14, 17 + storedAmount * 20);
    ...
    protected void renderGasTooltip(final MatrixStack stack, final String tooltip, final int mouseX,
            final int mouseY) {
