...
public class BriquettingPressScreen extends ContainerScreen<BriquettingPressContainer> {
    ...
    protected void drawGuiContainerForegroundLayer(final MatrixStack stack, final int mouseX,
        ...
        if (tileEntity.buttonPressed) {
            final StringBuilder eta = new StringBuilder("Completed in: ");
            if (Math.floor((double) tileEntity.getMaxRunningTime() / 1200) != 0) {
                eta.append(Double.toString(Math.floor((double) tileEntity.getMaxRunningTime() / 1200))
                        .replace(".0", "")).append(" mins and ")
                        .append(Double.toString((double) tileEntity.getMaxRunningTime() % 1200 / 20)
                                .replace(".0", ""))
                        .append(" secs!");
            } else {
    ...
    protected void init() {
        ...
        this.addButton(new RunButton(this.guiLeft + 132, this.guiTop + 35, 21, 18, button -> {
            final BriquettingPressTileEntity tile = getContainer().tileEntity;
    ...
    public static class RunButton extends Button {
        ...
        public RunButton(final int xPos, final int yPos, final int width, final int height,
        ...
        public void renderWidget(final MatrixStack stack, final int mouseX, final int mouseY,
            ...
            if (!isMouseOver(mouseX, mouseY)) {
                ...
                ClientUtils.renderButton(stack, BUTTON_TEXTURES, this, partialTicks, this.alpha, 0, 17,
                        this.width, this.height, 64, 64);
            }
        }
    }
