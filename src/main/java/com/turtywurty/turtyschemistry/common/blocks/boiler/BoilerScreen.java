...
public class BoilerScreen extends ContainerScreen<BoilerContainer> {
    ...
    public void render(final MatrixStack stack, final int mouseX, final int mouseY,
        ...
        this.renderTooltip(stack, this.container.getInputFluid().getDisplayName().getString(), mouseX, mouseY,
                26, 13, 40, 71);
        this.renderTooltip(stack, this.container.getOutputFluid().getDisplayName().getString(), mouseX,
                mouseY, 116, 13, 130, 71);
        this.renderTooltip(stack,
                new TranslationTextComponent(this.container.getOutputGas().getTranslationKey()).getString(),
    ...
    protected void drawGuiContainerForegroundLayer(final MatrixStack stack, final int mouseX,
        ...
        if (this.container.getOutputGas() instanceof GasBlock) {
            ClientUtils.drawTexture(((GasBlock) this.container.getOutputGas()).getGuiTexture(), 153, 14, 13,
                    57, 1000, 1000);
        }
    }

    protected void renderTooltip(final MatrixStack stack, final String tooltip, final int mouseX,
            final int mouseY, final int xLeft, final int yTop, final int xRight, final int yBottom) {
