...
public class ElectrolyzerContainer extends Container {
    ...
    public ElectrolyzerContainer(final int id, final PlayerInventory playerInventory,
            final IItemHandler slots, final BlockPos pos, final IIntArray data) {
    ...
    public static ElectrolyzerContainer getClientContainer(final int id, final PlayerInventory playerInv) {
        return new ElectrolyzerContainer(id, playerInv, new ItemStackHandler(3), BlockPos.ZERO,
                new IntArray(5));
    }
                ...
                for (final IContainerListener listener : this.listeners) {
                    if (listener instanceof ServerPlayerEntity) {
                        TurtyChemistry.PACKET_HANDLER.send(
                                ...
                                new ElectrolyzerFluidPacket(tileEntity.fluidInventory.getFluidInTank(0),
                                        this.windowId, 0));
                    }
                }
            }
            ...
            if (!this.outputFluid1.isFluidStackIdentical(tileEntity.fluidInventory.getFluidInTank(1))) {
                ...
                for (final IContainerListener listener : this.listeners) {
                    if (listener instanceof ServerPlayerEntity) {
                        TurtyChemistry.PACKET_HANDLER.send(
                                PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) listener),
                                new ElectrolyzerFluidPacket(tileEntity.fluidInventory.getFluidInTank(1),
                                        this.windowId, 1));
                    }
                }
            }
            ...
            if (!this.outputFluid2.isFluidStackIdentical(tileEntity.fluidInventory.getFluidInTank(2))) {
                ...
                for (final IContainerListener listener : this.listeners) {
                    if (listener instanceof ServerPlayerEntity) {
                        TurtyChemistry.PACKET_HANDLER.send(
                                PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) listener),
                                new ElectrolyzerFluidPacket(tileEntity.fluidInventory.getFluidInTank(2),
                                        this.windowId, 2));
                    }
                }
            }
