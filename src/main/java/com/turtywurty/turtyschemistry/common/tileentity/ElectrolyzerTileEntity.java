...
public class ElectrolyzerTileEntity extends InventoryTile implements INamedContainerProvider {
	...
	private final int maxRunningTime = 40;
	private final int maxHydrogen = 10000;
	private final int maxOxygen = 10000;
	private final int maxWater = 10000;
	private final int bucketHydrogen = 500;
	private final int bucketOxygen = 250;
	...
	public Container createMenu(int windowID, PlayerInventory playerInv, PlayerEntity player) {
		return new ElectrolyzerContainer(windowID, playerInv, this);
	}
		...
		if (this.getItemInSlot(2).getItem().equals(ItemInit.GAS_CANISTER_S.get())
			...
			if (nbt.contains("GasName")) {
				if (nbt.getString("GasName").equalsIgnoreCase("turtychemistry:oxygen")) {
					if (nbt.getInt("GasStored") < nbt.getInt("MaxAmount") && this.storedOxygen > 0) {
						nbt.putInt("GasStored", nbt.getInt("GasStored") + 1);
						this.storedOxygen--;
					}
				}
