...
public class FractionalDistillerTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	public boolean isValidOutput(ItemStack item, int slot) {
		if (slot == REFGAS_SLOT && item.getItem() == ItemInit.REFINARY_GAS.get())
			return true;
		if (slot == PETROL_SLOT && item.getItem() == ItemInit.PETROL.get())
			return true;
		if (slot == NAPHTHA_SLOT && item.getItem() == ItemInit.NAPHTHA.get())
			return true;
		if (slot == KEROSINE_SLOT && item.getItem() == ItemInit.KEROSINE.get())
			return true;
		if (slot == DIESEL_SLOT && item.getItem() == ItemInit.DIESEL.get())
			return true;
		if (slot == LUBOIL_SLOT && item.getItem() == ItemInit.LUBRICATING_OIL.get())
			return true;
		if (slot == FUELOIL_SLOT && item.getItem() == ItemInit.FUEL_OIL.get())
			return true;
		if (slot == BITUMEN_SLOT && item.getItem() == ItemInit.BITUMEN.get())
			return true;
		else
			return false;
	}
	...
	public void tick() {
		if (world == null || world.isRemote)
			return;
		if (ticks == 0 && world != null && !world.isRemote)
			...
			if (canInsertResultIntoOutput) {
				...
				if (powered) {
					if (processTimeLeft == -1) {
						...
						System.out.println("processing time is not -1, it is:" + processTimeLeft);
						--processTimeLeft;
						if (processTimeLeft == 0) {
							System.out.println("processing time is 0");
							...
							System.out.println("put items in slots");
							if (input.hasContainerItem()) {
								System.out.println("input has container item");
		...
		if (lastProcessing != powered) {
			System.out.println("lastProcessing is not equal to powered");
			this.markDirty();
