...
public class BalerTileEntity extends LockableLootTileEntity
	...
	private final String[] recipes = new String[] { "minecraft:string", "minecraft:string", "minecraft:wheat",
	...
	public void tick() {
		...
		if (this.completed && this.itemTicksExisted < 200) {
			...
			if (this.itemTicksExisted >= 200) {
				...
				Direction facing = this.getBlockState().get(BalerBlock.HORIZONTAL_FACING);
	...
	public boolean isFull() {
		...
		for (ItemStack stack : this.items) {
			if (stack.isEmpty())
				return false;
			if (i <= 9) {
				if (!this.items.get(i).getItem().equals(Items.WHEAT)) {
					return false;
				}
			} else if (i > 9 && i <= this.getSizeInventory()) {
				if (!this.items.get(i).getItem().equals(Items.STRING)) {
					return false;
				}
			}
			i++;
