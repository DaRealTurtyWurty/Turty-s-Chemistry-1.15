...
import com.turtywurty.turtyschemistry.client.util.CombinedModelData;
import com.turtywurty.turtyschemistry.client.util.SinglePropertyModelData;
import com.turtywurty.turtyschemistry.core.util.BlockstateProvider;
import com.turtywurty.turtyschemistry.core.util.IDirectionalTile;
import com.turtywurty.turtyschemistry.core.util.IPropertyPassthrough;
import com.turtywurty.turtyschemistry.core.util.ModBlockProperties;
...
public abstract class BaseTileEntity extends TileEntity implements BlockstateProvider {

	private BlockState overrideBlockState = null;

	public BaseTileEntity(TileEntityType<? extends TileEntity> type) {
		super(type);
	}
	...
	public void rotate(Rotation rot) {
		if (rot != Rotation.NONE && this instanceof IDirectionalTile
				&& ((IDirectionalTile) this).canRotate(Direction.UP)) {
	...
	public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
		/*if (cap == CapabilityEnergy.ENERGY && this instanceof EnergyHelper.IIEInternalFluxConnector) {
			if (!energyCaps.containsKey(side)) {
				IEForgeEnergyWrapper wrapper = ((EnergyHelper.IIEInternalFluxConnector) this)
						.getCapabilityWrapper(side);
				if (wrapper != null)
					energyCaps.put(side, registerConstantCap(wrapper));
				else
					energyCaps.put(side, LazyOptional.empty());
			}
			return energyCaps.get(side).cast();
		}*/
	...
	public void updateContainingBlockInfo() {
		...
		if (old != null && newState != null && getType().isValidBlock(old.getBlock())
	...
	public IModelData getModelData() {
		...
		if (this instanceof IPropertyPassthrough)
			return new CombinedModelData(base, new SinglePropertyModelData<>(this, ModBlockProperties.TILEENTITY_PASSTHROUGH));
		else
			return base;
	}
}
