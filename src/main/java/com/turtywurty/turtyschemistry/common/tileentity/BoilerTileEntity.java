...
public class BoilerTileEntity extends InventoryTile {
	...
	private List<BoilerRecipe> getRecipes() {
		List<BoilerRecipe> rData = new ArrayList<BoilerRecipe>();
		TurtyChemistry.BOILER_RECIPE.getData().forEach((name, data) -> {
			rData.add(data);
		});
		return rData;
	}
	...
	public void read(CompoundNBT compound) {
		...
		this.outputGas = Registry.BLOCK
				.getOrDefault(new ResourceLocation(compound.getString("OutputGas"))) instanceof Block
						? (Block) Registry.BLOCK.getOrDefault(new ResourceLocation(compound.getString("OutputGas")))
