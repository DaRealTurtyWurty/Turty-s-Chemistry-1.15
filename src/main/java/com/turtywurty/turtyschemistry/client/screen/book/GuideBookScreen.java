...
public class GuideBookScreen extends Screen {
	...
	public class BackPageButton extends Button {
		...
		public BackPageButton(final GuideBookScreen screen, final int widthIn, final int heightIn, final int width,
				final int height, final int textureX, final int textureY, final Button.IPressable ipressable) {
		...
		public void renderWidget(final MatrixStack stack, final int mouseX, final int mouseY,
			...
			if (isHovered()) {
				if (this.screen.currentPage != 0) {
					this.active = true;
					ClientUtils.renderButton(stack, PAGE_TEXTURE, this, partialTicks, 1.0f, this.texX + 23, this.texY,
							18, 10, 256, 256);
				} else {
					this.active = false;
				}
			} else if (this.screen.currentPage != 0) {
				ClientUtils.renderButton(stack, PAGE_TEXTURE, this, partialTicks, 1.0f, this.texX, this.texY, 18, 10,
						256, 256);
			}
		}
	}
	...
	public class NextPageButton extends Button {
		...
		public NextPageButton(final GuideBookScreen screen, final int widthIn, final int heightIn, final int width,
				final int height, final int textureX, final int textureY, final Button.IPressable ipressable) {
			super(widthIn, heightIn, width, height, new StringTextComponent(""), ipressable);
			this.texX = textureX;
			this.texY = textureY;
		...
		public void renderWidget(final MatrixStack stack, final int mouseX, final int mouseY,
			...
			if (isHovered()) {
				...
				ClientUtils.renderButton(stack, PAGE_TEXTURE, this, partialTicks, 1.0f, this.texX + 23, this.texY, 18,
						10, 256, 256);
			} else {
				if (this.screen.currentPage == 0) {
					this.x = (this.screen.width - 125) / 2 + 98;
	...
	public void render(final MatrixStack stack, final int mouseX, final int mouseY, final float partialTicks) {
		...
		for (Widget element : this.buttons) {
	...
	public void renderContentsPage(final MatrixStack stack, final int mouseX, final int mouseY,
		...
		if (ClientUtils.isMouseInArea(mouseX, mouseY, this.guiLeft + 11, this.guiTop + 21, 50, 5)) {
			StringTextComponent text = new StringTextComponent("• Elements");
