...
public class SiloScreen extends ContainerScreen<SiloContainer> {
	...
	protected void init() {
		super.init();
		this.addButton(new NextPageButton(this.guiLeft + 229, this.guiTop + 164, 16, 11, (button) -> {
			TurtyChemistry.packetHandler.send(PacketDistributor.SERVER.noArg(), new SiloButtonPacket(1));
		}, this.container));

		this.addButton(new BackPageButton(this.guiLeft + 13, this.guiTop + 164, 16, 11, (button) -> {
			TurtyChemistry.packetHandler.send(PacketDistributor.SERVER.noArg(), new SiloButtonPacket(0));
		}, this.container));
	}
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		ClientUtils.blit((SiloScreen) this, this.guiLeft, this.guiTop, 0, 0, this.xSize, this.ySize);
	}
	...
	public static class NextPageButton extends Button {
		...
		public NextPageButton(int xPos, int yPos, int width, int height, IPressable onPress,
				SiloContainer containerIn) {
		...
		public void renderButton(int mouseX, int mouseY, float partialTicks) {
			...
			if (!this.isMouseOver(mouseX, mouseY)) {
				ClientUtils.renderButton(BUTTON_TEXTURES, this, partialTicks, this.alpha, 16, 0, this.width,
						this.height, 64, 64);
			} else {
				ClientUtils.renderButton(BUTTON_TEXTURES, this, partialTicks, this.alpha, 16, 11, this.width,
						this.height, 64, 64);
			}
		}
	}
	...
	public static class BackPageButton extends Button {
		...
		public BackPageButton(int xPos, int yPos, int width, int height, IPressable onPress,
				SiloContainer containerIn) {
			super(xPos, yPos, width, height, "", onPress);
			this.container = containerIn;
		}
		...
		public void renderButton(int mouseX, int mouseY, float partialTicks) {
			...
			if (!this.isMouseOver(mouseX, mouseY)) {
				...
				ClientUtils.renderButton(BUTTON_TEXTURES, this, partialTicks, this.alpha, 0, 11, this.width,
						this.height, 64, 64);
			}
		}
	}
